...
class CanvasDisplay {
    constructor() {
        ...
        this.drawWindow = (window, active) => {
            ...
            if (window.name === global.name) this.cx.fillText(window.name + ' | ' + global.users.length + ' users, ' + global.getRoomList().length + ' rooms', window.pos.x + 12, window.pos.y + 20);
            ...
            } else if (window instanceof ChessWindow) {
                this.cx.fillStyle = "#fff";

                this.cx.textAlign = "center";
                this.cx.fillText(
                    global.roomPlayer1 + ' VS ' + global.roomPlayer2,
                    window.pos.x + 16 * 5 + 8,
                    window.pos.y + 16 * 12 + 4
                );

                var xOrigin = 24;
                var yOrigin = 48;
                this.cx.fillRect(xOrigin - 5, yOrigin - 5, 8 * 16 + 10, 8 * 16 + 10);
                this.cx.fillStyle = "#000";
                this.cx.fillRect(xOrigin - 3, yOrigin - 3, 8 * 16 + 6, 8 * 16 + 6);
                for (let x = xOrigin; x < xOrigin + 8 * 16; x += 16) {
                    for (let y = yOrigin; y < yOrigin + 8 * 16; y += 16) {
                        if ((x - xOrigin) / 16 % 2 === 0 && (y - yOrigin) / 16 % 2 === 0 || (x - xOrigin) / 16 % 2 !== 0 && (y - yOrigin) / 16 % 2 !== 0) {
        ...
        this.drawFrame = () => {
            ...
            if (global.start && !global.connected) {
                this.cx.clearRect(0, 0, this.canvas.width, this.canvas.height);
                this.cx.fillStyle = "#fff";
                this.cx.font = "16px rcr";
                this.cx.textAlign = "center";
